...
| staging buffer: small ordinary getMem of the QP owner, address exchanged at QP setup | NCCL per-connection transport buffer (plain cudaMalloc, address exchanged at connect/accept, remote LL/FIFO writes land there for interpretation) | ours is degenerate: with loom_rx interposing, the staging address is a routing tag and the bytes may never rest in memory (G3 decides whether they also land) |
