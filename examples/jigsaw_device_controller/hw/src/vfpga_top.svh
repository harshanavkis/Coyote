 ...
 * Incoming RDMA WRITEs (from host) arrive on axis_rrsp_recv[0] → txn_generator_in
 * Outgoing RDMA WRITEs (to host) leave via txn_generator_out → axis_rreq_send[0]
 * RDMA WRITE submissions use APP_WRITE with RDMA_MODE_PARSE for auto-fragmentation.
 */
...
txn_generator inst_txn_generator (
    ...
    // Incoming data ← RDMA responder recv channel (incoming RDMA WRITEs from remote)
    ...
    // Outgoing data → RDMA request send channel (outgoing RDMA WRITEs)
