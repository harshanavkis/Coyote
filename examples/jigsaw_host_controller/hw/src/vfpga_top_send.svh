 ...
 * Jigsaw vFPGA Top
 *
 * Uses RDMA SEND as transport. The jigsaw protocol handles all addressing
 * internally — the RoCE stack is just a pipe. RDMA SEND submissions use
 * APP_SEND with RDMA_MODE_PARSE for automatic PMTU fragmentation.
 * PID is set once at init via MMIO.
 */
...
jigsaw_host_controller #(
    ...
    // Network side — incoming RDMA SENDs from remote
    ...
    // Network side — outgoing RDMA SENDs
